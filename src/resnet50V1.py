...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # output = self.forward_backbone(input_img.cuda(non_blocking=True))
        # all_outputs.append(output)

            # Additional branch for the 9 splits
            H, W = input_img.shape[-2], input_img.shape[-1]
            crops = []
            crop_size = H // 3  # Assuming H is divisible by 3
            for i in range(3):
                for j in range(3):
                    crops.append(input_img[..., i * crop_size:(i + 1) * crop_size, j * crop_size:(j + 1) * crop_size])

            encoded_crops = [self.forward_backbone(crop.cuda(non_blocking=True)) for crop in crops]

            # Logic to combine these 9 encoded crops into one tensor.
            decoded_output = sum(encoded_crops) / len(encoded_crops)
            # decoded_output = self.decoder(decoded_output)

            # len(output) = 96
        return self.forward_head(output), self.forward_head(decoded_output)
