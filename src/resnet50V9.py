...
class ResNet(nn.Module):
    def __init__(
        ...
        self.fc_rot = nn.Linear(512 * block.expansion, 4)
        ...
        self.side4 = nn.Conv2d(in_channels=64, out_channels=1, kernel_size=3, stride=2, padding=0)
        self.side2 = nn.Conv2d(in_channels=256, out_channels=1, kernel_size=3, stride=2, padding=0)
        self.side3 = nn.Sequential(nn.Conv2d(in_channels=512, out_channels=1, kernel_size=2, stride=1, padding=0))
        # self.side5 = nn.Sequential(nn.Conv2d(2048, 16, 1, stride=1, bias=True),
        #                            nn.ConvTranspose2d(in_channels=16, out_channels=16, kernel_size=4, stride=2, padding=1),
        #                            nn.ConvTranspose2d(in_channels=16, out_channels=16, kernel_size=5, stride=3, padding=1)
        #                            )
        self.side5 = nn.Sequential(nn.Conv2d(2048, 8, 1, stride=1, bias=True),
                                   nn.ConvTranspose2d(in_channels=8, out_channels=8, kernel_size=6, stride=3,padding=1)
)
        self.fuse = nn.Conv2d(8 * 4, 8, 1, groups=8, bias=True)
    ...
    def forward_backbone(self, x, low_feature=False):
        ...
        if low_feature:
            # print("c1", c1.shape)
            # torch.Size([96, 64, 34, 34])
            side4 = self.side4(c4)
            # print("c2", c2.shape)
            # torch.Size([96, 256, 34, 34])
            side2 = self.side2(c2)
            # print("c3", c3.shape)
            # torch.Size([96, 512, 17, 17])
            side3 = self.side3(c3)
            # print("c5", c5.shape)
            # torch.Size([96, 2048, 5, 5])
            side5 = self.side5(c5)

            # torch.Size([96, 16, 34, 34])
            # print("side5", side5.shape)

            slice5 = side5[:, 0:1, :, :]
            # print("slice5", slice5.shape)
            # print("side1", side1.shape)
            # print("side2", side2.shape)
            # print("side3", side3.shape)
            fuse = torch.cat((slice5, side4, side2, side3), 1)

            # print("fuse", fuse.shape)

            for i in range(side5.size(1) - 1):
                slice5 = side5[:, i + 1:i + 2, :, :]
                fuse = torch.cat((fuse, slice5, side4, side2, side3), 1)

            fuse = self.fuse(fuse)

            # print("fusebbbbbbbb", fuse.shape)

            x = torch.flatten(fuse, 1)
            # print("x", x.shape)
            x = torch.flatten(x, 1)
            return x
    ...
    def forward_head(self, x):
        # print("self.projection_head is not None", self.projection_head is not None)
        if self.projection_head is not None:
            x = self.projection_head(x)
        # print("self.l2norm", self.l2norm)
        if self.l2norm:
            x = nn.functional.normalize(x, dim=1, p=2)
        # print("self.prototypes is not None", self.prototypes is not None)
    ...
    def forward(self, inputs, classification=False, low_feature=False):
        ...
        for end_idx in idx_crops:
            _out = self.forward_backbone(torch.cat(inputs[start_idx: end_idx]).cuda(non_blocking=True), low_feature=True)
            # h = self.predictor
            # _out = h(_out)
            if start_idx == 0:
                output = _out
        ...
        if not classification:
            ...
            # 创建一个新的列表来保存旋转后的图像分片
            rotated_fragments = []
            for fragment in [f1, f2, f3, f4]:
                # 应用旋转变换并存储旋转的角度
                rotated_fragment, rotation_idces = rotation_transform(fragment)
                rotated_fragments.append(rotated_fragment)
                label_ro.extend(rotation_idces)

            q_gather = torch.cat(rotated_fragments, dim=0)
            # q_gather = torch.Size([192, 3, 16, 16])
            q_gather = self.forward_backbone(q_gather.cuda())
            # 展平之前torch.Size([192, 2048, 3, 3])
            # q_gather.shape=torch.Size([192, 2048])

            # 我有192个2048维样本，每个样本输出0，1，2，3
            q_loc = self.fc_loc(q_gather)
            q_rot = self.fc_rot(q_gather)

            # 每个数字重复 bs_all 次，四个数字连续排列。[0, 0, 0, ..., 1, 1, 1, ..., 2, 2, 2, ..., 3, 3, 3, ...]
            label_loc = torch.LongTensor([0] * bs_all + [1] * bs_all + [2] * bs_all + [3] * bs_all).cuda()
            # 存放旋转角度
            label_rot = torch.LongTensor(label_ro).cuda()

            return self.forward_head(output), label_loc, q_loc, label_rot, q_rot
