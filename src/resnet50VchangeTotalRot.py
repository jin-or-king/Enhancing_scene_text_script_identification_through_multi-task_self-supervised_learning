...
class ResNet(nn.Module):
    ...
    def forward(self, inputs, classification=False):
        # swav
        # len(inputs) = 2
        ...
        if not classification:
            ...
            images_blocks = [f1, f2, f3, f4]
            # 随机排列图像块的顺序
            random.shuffle(images_blocks)
            # 按照随机顺序合并图像块
            row1 = torch.cat([images_blocks[0], images_blocks[1]], dim=3)
            row2 = torch.cat([images_blocks[2], images_blocks[3]], dim=3)
            final_images = torch.cat((row1, row2), dim=2)

            # 按照64这个维度进行循环
            rotated_fragments = []

            rotated_fragment, rotation_idces = rotation_transform(final_images)
            ...
            rotated_fragments = self.forward_backbone(rotated_fragments.cuda())
            ...
            return self.forward_head(output), q_rot, label_rot
