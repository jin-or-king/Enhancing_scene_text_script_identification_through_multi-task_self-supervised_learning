...
class ResNet(nn.Module):
    ...
    def forward(self, inputs, classification=False, low_feature=True):
        # swav
        # len(inputs) = 2
        ...
        for end_idx in idx_crops:
            _out = self.forward_backbone(torch.cat(inputs[start_idx: end_idx]).cuda(non_blocking=True), low_feature)
            # h = self.predictor
            # _out = h(_out)
            if start_idx == 0:
                output = _out
