...
class ResNet(nn.Module):
    def __init__(
        ...
        # 多头自注意力
        self.attention = Attention(dim=attention_dim, num_heads=num_heads)
    ...
    def forward(self, inputs, classification=False):
        ...
        if not classification:
            ...
            for images in inputs:
                ...
                # q_gather = torch.Size([bs * 4, 3, 16, 16])
                ...
                # q_gather = q_gather.view(bs_all * 4, 32, 64)  # torch.Size([192, 32, 64])
                # q_gather = self.attention(q_gather)
                # # torch.Size([192, 32, 64])
                # q_gather = torch.flatten(q_gather, 1)
                # # q_gather = torch.Size([192, 2048])
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        # x.shape = torch.Size([192, 32, 64])
        # self.qkv.weight = torch.Size([64, 16])
