...
class ResNet(nn.Module):
    ...
    def forward_backbone(self, x, low_feature):
        x = self.padding(x)
        ...
        if low_feature:
            ...
            feature_high = self.avgpool(feature_high)
    ...
    def forward(self, inputs, classification=False, low=False):
        # swav
        # len(inputs) = 2
        ...
        if not classification:
            ...
            rotated_fragment, rotation_idces = rotation_transform(images)
            rotated_fragments.append(rotated_fragment)
            rotated_fragments = torch.squeeze(torch.stack(rotated_fragments))

            label_ro.extend(rotation_idces)

            # 创建一个新的列表来保存旋转后的图像分片
            local_rotated_fragments = []
            label_combined = []
            fragments = [f1, f2, f3, f4]
            for i, fragment in enumerate(fragments):
                # 应用旋转变换并存储旋转的角度
                rotated_fragment, rotation_idces = rotation_transform(fragment)
                local_rotated_fragments.append(rotated_fragment)
                # 计算综合标签
            ...
            q_gather = torch.cat(local_rotated_fragments, dim=0)
            q_gather = self.forward_backbone(q_gather.cuda(), low)
            # q_gather现在是一个展平的特征图

            rotated_fragments = self.forward_backbone(rotated_fragments.cuda(), low)
            # 展平之前torch.Size([192, 2048, 3, 3])
            # q_gather.shape=torch.Size([192, 2048])

            # 分类处理 - 修改为输出16个分类
            q_class = self.fc_class(q_gather)  # 假设fc_class是一个新的全连接层，输出16个分类
            q_rot = self.fc_rot(rotated_fragments)
            ...
            return self.forward_head(output), label_combined, q_class, label_rot, q_rot
