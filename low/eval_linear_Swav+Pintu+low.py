...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            torch.save(save_dict, os.path.join(args.dump_path, "checkpoint_Swav+Pintu+low_3meth1.pth.tar"))
...
def train(model, reglog, optimizer, loader, epoch, classification=True):
    ...
    for i in range(0, len(loader)):
        ...
        for iter_epoch, (inp, target) in enumerate(loader[i]):
            ...
            # forward
            with torch.no_grad():
                output = model(inp, classification, low_feature=True)
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(val_loader)):
            ...
            for j, (inp, target) in enumerate(val_loader[i]):
                ...
                # compute output
                output = linear_classifier(model(inp, classification, low_feature=True))
                loss = criterion(output, target)
