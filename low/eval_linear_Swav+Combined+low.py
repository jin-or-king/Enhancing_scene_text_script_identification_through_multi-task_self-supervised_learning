...
def main():
    ...
    restart_from_checkpoint(
        os.path.join(args.dump_path, "checkpoint_swav+combined+low_all.pth.tar"),
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            torch.save(save_dict, os.path.join(args.dump_path, "checkpoint_swav+combined+low_all.pth.tar"))
...
def plot_confusion_matrix(cm, classes, savename, title, normalize=True, cmap=plt.cm.Blues, fontsize=5):
    """
    - cm : 计算出的混淆矩阵的值
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        ...
        # 设置一个最小阈值，避免显示为零
        if normalize and value < 0.0001:
            value = 0  # 或者使用一个很小的数字，如 0.0001
