...
class MyData(Dataset):
    ...
    def __getitem__(self, idx):    # Get through here every iteration when refreshing the dataloader
        ...
        # (image1, image2) = self.image_process(image)
        # if self.fusion:
...
def getloaders(root_dir, mid_dir, widths, txts, transf, shuffle=True, bs=32, aug=False, pad=False,
    ...
    for i in range(len(widths)):
        ...
        loaders.append(DataLoader(mydataset, batch_size=bs, shuffle=shuffle, num_workers=0))
    return loaders
...
class Transforms():
    ...
    def __call__(self, inp):
        image = self.test_transform(inp)

        return image


def build_dataset(batch_size, root_dir, mid_dir):
    ...
    temp_dir = os.path.join(mid_dir, "testset/")
    ...
    test_loader = getloaders(root_dir, temp_dir, widths, txts, train_transform, shuffle=False, bs=batch_size, aug=True,
                           pad=False, multilabel=False, triplet=False)

    temp_dir = os.path.join(mid_dir, "trainset/")
    train_loader = getloaders(root_dir, temp_dir, widths, txts, train_transform, shuffle=True, bs=batch_size, aug=True,
                           pad=False, multilabel=False, triplet=False)
    return train_loader, test_loader, num_classes

if __name__ == "__main__":
    root_dir = 'E:\\data\\data\\SIW\\dataset\\SIW-13'
    mid_dir = "E:\\data\\data\\SIW\\dataset\\SIW-13\\z_grp_ccx\\testset"
    ...
    # dataiter = iter(myloaders)
    images, label = next(iter(myloaders[1]))
    print(len(label))
    print(len(images))
    # images, label, _, label_pos, _, label_neg = dataiter.next()
    print(images[2].size(), label)
    print(images[6].size(), label)
